    ...
    if (section != m_mSections.end())
    {
        ...
        if (item.first != item.second)
        {
            if (distance(item.first, item.second) > 1)
                OutputDebugString(wstring(L"Warnung: Configfile has hidden entrys in section \'" + strSektion + L"\', key \'" + strValue + L"\' exist more than once").c_str());
...
void ConfFile::LoadFile(const wstring& strFilename)
{
    ...
    function<void(const wstring&)> fnLoadFileRecrusive = [&](const wstring& strFilename)
    {
        ...
        if (fin.is_open() == true)
        {
            fin.imbue(locale("en_US.UTF-8")); //fin.imbue(std::locale(fin.getloc(), new codecvt_byname<wchar_t, char, mbstate_t>("en_US.UTF-8")));
            ...
            OutputDebugString(wstring(L"Error: Configfile \'" + strFilename + L"\' could not be opened").c_str());
    };
